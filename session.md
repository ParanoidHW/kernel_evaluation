...
- Current GMM validation: Gemma large/occupied rows are all within routing bounds; Longcat is above the balanced bound by up to about 54%; DS3.2 INT8 GMM is above by up to about 18%. These above-bound rows remain low confidence until real routing, expert scheduling, sync/merge, or GMM-specific kernel source details are available.

...
当 profiling 数据来自昇腾平台，但目录 README 或外部元数据没有明确 SoC 时，可用 `Block Dim`/`Block Num` 做初步交叉判断。该规则只适用于 Ascend/CANN profiling，不应外推到其他硬件平台。

- 先按算子类型区分 Cube 类与 Vector 类，不要直接使用全文件最大 block dim。
- Cube 类算子主要参考 `aic_num`，例如 `MatMul`、`BatchMatMul`、`GroupedMatmul`、`QuantBatchMatmul`，以及以 Cube 路径为主的 attention/flash-attention 子路径。
- Vector 类算子主要参考 `aiv_num`，例如 elementwise、cast、transpose、rotary、gather/scatter、activation、normalization、routing、部分 fusion 和搬运类算子。
- 对 910B4，常见交叉证据是 Cube 最大 block dim 约为 `20`，Vector 最大 block dim 约为 `40`。
- 对 910C/A3，常见交叉证据是 Cube 最大 block dim 约为 `24`，Vector 最大 block dim 约为 `48`。
- attention 的 block dim 可能因为 tiling、head 数、序列长度、KV cache、稀疏或量化路径显著小于平台 `aic_num`；不能仅凭单个 attention kernel 的较小 block dim 否定平台判断。
- `Block Dim=0`、host 侧行、通信行或统计字段缺失行不能作为平台核数证据。

若 Cube 证据和 Vector 证据冲突，应优先检查算子分类、profiling 字段含义、是否为混合平台数据、是否存在通信/融合算子被误归类为计算算子，并在方案中标注 `platform_inference_low_confidence`。

...
- other_ops 本轮已补充 `DynamicQuant`、`AddRmsNormDynamicQuant`、`RotaryMul`、`MlaPrologV3`、`ReverseV2`、`ReduceAny`、`ArgMaxWithValue`、`QuantLightningIndexer`。longcat 910C other_ops resolved `2210`、unresolved `160`；ds3.2 other_ops resolved `3770`、unresolved `80`。`LightningIndexerQuant` 通过评估入口 alias 映射到 `QuantLightningIndexer`，该 alias 不是硬件配置，可关闭。剩余未支持项中 `Data` 是框架占位，`AutomaticBufferFusionOp` 为用户确认的非固定融合包装。
...
8. other_ops `QuantLightningIndexer`：已通过 `LightningIndexerQuant -> QuantLightningIndexer` alias 支持，但缺 exact `sparse_count/sparse_mode/actual_seq_lengths/block_table` runtime 值；当前保持 source-strategy low-confidence。
